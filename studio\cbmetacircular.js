...
function rearrange(body) {
        const functions = filter(item => head(item) === 'function_declaration' ,body);
        const not_functions = filter(item => head(item) !== 'function_declaration', body);
        const rearranged = append(functions, not_functions);
        return rearranged;
}
function find_names(program) {
    let names = null;
    function helper(program) {
        // if (is_null(program)) {
        //     return names;
        // }
        if (head(program) === 'name') {
            const name = list_ref(program, 1);
            names = pair(name, names);
        } else if (is_list(head(program))) {
            helper(head(program));
            helper(tail(program));
        }
    }
...
function evaluate(program) {
    ...
    while (! is_null(C)) {
        ...
        C = tail(C);
        const declarations = scan_out_declarations(command);
        D = pair(declarations, D);
        ...
        if (is_literal(command)) {
            ...
            const seq = sequence_statements(command);
            const body = rearrange(seq);
...
`;
const test = `false ? abracadabra(simsalabim) : 42;`;
const test1 = `ababa; keke;`;
// parse_and_evaluate(baba);
// parse_and_evaluate(test);
parse_and_evaluate(test);
// scan_out_declarations(pars
// parse(test);
// display_list(parse(test));
// parse(test1);
display_list(find_names(parse(test1)));
// rearrange(parse(`
// f(49);
// function f(x) {
//     12;
